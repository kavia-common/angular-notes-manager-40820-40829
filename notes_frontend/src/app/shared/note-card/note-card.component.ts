    ...
    <article class="card">
      <header class="card__header">
        ...
        <div class="card__actions">
          ...
          <a class="btn btn--ghost" [routerLink]="['/notes', note.id]" (click)="view.emit(note)" title="View" aria-label="View note">👁</a>
          <a class="btn btn--ghost" [routerLink]="['/notes', note.id, 'edit']" (click)="edit.emit(note)" title="Edit" aria-label="Edit note">✏️</a>
          <button class="btn btn--danger" (click)="delete.emit(note)" title="Delete" aria-label="Delete note">🗑</button>
        </div>
      </header>
      <section class="card__body" (click)="view.emit(note)" role="link" tabindex="0">
...
.card__title { font-size: 1rem; font-weight: 600; color: var(--op-text, #111827); margin: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; cursor: pointer; }
